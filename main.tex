...
where we scaled $F_{obs}$ by  $\sigma_F$  (i.e. $F_{mean} = x_{mean} \cdot \sigma_{F}$). In this and following calculations we used  scipy implementation of 
often used integrals of Gaussian distributions : 

- cumulative density function, which is an area under a Gaussian distribution from $-\infty$ to $x_{0}$ :

\begin{equation}
\cdf{x_{0}} = \int_{-\infty}^{x_{0}}{\mathcal{N}(\mu,\sigma)} = \int_{-\infty}^{x_{0}}{\frac{1}{\sqrt{2\pi\sigma^{2}}} \exp{(x-\mu)^{2} / 2\sigma^{2}}}
\end{equation}
...
\begin{multline}
\sf(x_{0}) =  \int_{x_{0}}^{\infty}{\mathcal{N}(\mu,\sigma)} =  \int_{-\infty}^{\infty}{\mathcal{N}(\mu,\sigma)} -  \int_{-\infty}^{x_{0}}{\mathcal{N}(\mu,\sigma)} = \\
1 - \cdf(x_{0})
\end{multline}
...
This expression for $z_{med}$ can be evaluated : rhs is  a survival function  : 
